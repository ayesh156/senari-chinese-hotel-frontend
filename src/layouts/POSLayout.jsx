...
/**
 * allowedRoles — undefined means all authenticated roles can see it.
...
const NAV_ITEMS = [
  ...
  { to: '/pos/foods',            icon: UtensilsCrossed, label: 'Foods',            allowedRoles: ['ADMIN', 'MANAGER'] },
  { to: '/pos/inventory',        icon: Package,         label: 'Inventory'        },
  { to: '/pos/master-data',      icon: Database,        label: 'Master Data',      allowedRoles: ['ADMIN', 'MANAGER'] },
  { to: '/pos/customers',        icon: Users,           label: 'Customers'        },
  { to: '/pos/suppliers',        icon: Truck,           label: 'Suppliers',        allowedRoles: ['ADMIN', 'MANAGER'] },
  { to: '/pos/purchase-orders',  icon: ShoppingCart,    label: 'Purchase Orders',  allowedRoles: ['ADMIN', 'MANAGER'] },
  { to: '/pos/tables',           icon: LayoutGrid,      label: 'Tables'           },
  { to: '/pos/reports',          icon: BarChart2,       label: 'Reports',          allowedRoles: ['ADMIN', 'MANAGER'] },
  { to: '/pos/settings',         icon: Settings,        label: 'Settings',         allowedRoles: ['ADMIN'] },
]
...
// ── Sidebar ───────────────────────────────────────────────────────────────────
function Sidebar({ collapsed, onToggleCollapse, onClose, onLogout, staff, userRole }) {
    ...
    <div className="flex flex-col h-full">
      ...
      <nav className={`flex-1 py-4 flex flex-col gap-1 overflow-y-auto
        ...
        {visibleItems.map(item => {
          // Omit allowedRoles from SideNavLink props
          const { allowedRoles, ...linkProps } = item
          return (
            <SideNavLink
              key={linkProps.to}
              {...linkProps}
