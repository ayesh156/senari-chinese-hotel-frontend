...
function CategoryPill({ category }) {
  const cls = CAT_COLORS[category] ?? 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 border-gray-200 dark:border-gray-700'
...
function SupplierFormModal({ initialData, onSave, onCancel }) {
  ...
  function handleSubmit(e) {
    ...
    onSave({ name: name.trim(), phone: phone.trim(), email: email.trim(), address: address.trim(), category })
  }

  const inputCls = (hasErr) => `w-full px-3 py-2.5 rounded-xl text-sm bg-white dark:bg-gray-800 border text-gray-900 dark:text-gray-100 placeholder:text-gray-400 dark:placeholder:text-gray-600 focus:outline-none focus:ring-2 transition-colors ${hasErr ? 'border-red-400 focus:ring-red-400/30' : 'border-gray-200 dark:border-gray-700 focus:ring-amber-400/40'}`
  const FieldLabel = ({ children }) => <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide block mb-1.5">{children}</label>

  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="rounded-t-2xl sm:rounded-2xl w-full max-w-2xl shadow-2xl border bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700/50 max-h-[90vh] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className={`relative overflow-hidden shrink-0 sticky top-0 z-10 ${isEdit ? 'bg-gradient-to-r from-emerald-600 via-teal-500 to-teal-600' : 'bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600'}`}>
          <div className="relative flex items-center gap-3 px-4 sm:px-5 py-4">
            <div className="w-10 h-10 bg-white/20 backdrop-blur rounded-xl flex items-center justify-center shrink-0"><Truck size={18} className="text-white" /></div>
            <div className="flex-1 min-w-0"><h2 className="text-base font-bold text-white truncate">{isEdit ? `Edit — ${initialData.name}` : 'Add New Supplier'}</h2><p className="text-white/70 text-xs mt-0.5">{isEdit ? 'Update supplier details' : 'Register a vendor for purchases'}</p></div>
          ...
          <div className="flex-1 min-h-0 overflow-y-auto px-4 sm:px-5 py-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                ...
                <input type="text" value={name} autoFocus onChange={e => { setName(e.target.value); setErrors(p => ({ ...p, name: '' })) }} placeholder="e.g. Perera Groceries" className={inputCls(!!errors.name)} />
              ...
              <div>
                ...
                <input type="tel" value={phone} onChange={e => { setPhone(e.target.value); setErrors(p => ({ ...p, phone: '' })) }} placeholder="077 123 4567" className={inputCls(!!errors.phone)} />
              ...
              <div>
                ...
                <input type="email" value={email} onChange={e => { setEmail(e.target.value); setErrors(p => ({ ...p, email: '' })) }} placeholder="supplier@example.com" className={inputCls(!!errors.email)} />
                {errors.email && <p className="text-xs text-red-500 mt-1">{errors.email}</p>}
              </div>
              <div className="sm:col-span-2">
                <FieldLabel>Address</FieldLabel>
                <textarea rows={2} value={address} onChange={e => setAddress(e.target.value)} placeholder="Full address…" className={`${inputCls(false)} resize-none`} />
              </div>
            </div>
          </div>
          <div className="shrink-0 px-4 sm:px-5 py-4 border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 flex gap-3 sticky bottom-0 z-10">
            <button type="submit" className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl font-semibold text-sm text-white shadow-md hover:opacity-90 ${isEdit ? 'bg-gradient-to-r from-emerald-500 to-teal-500 shadow-emerald-500/20' : 'bg-gradient-to-r from-amber-500 to-orange-500 shadow-amber-500/20'}`}>
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="rounded-2xl max-w-md w-full shadow-2xl border overflow-hidden bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700/50">
        <div className="p-6 border-b bg-gradient-to-r from-red-100 to-red-50 dark:from-red-600/20 dark:to-red-500/10 border-red-200 dark:border-red-500/30">
          <div className="flex items-center gap-4"><div className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0 bg-red-100 dark:bg-red-500/20"><AlertTriangle size={22} className="text-red-600 dark:text-red-400" /></div><h2 className="text-xl font-bold text-gray-900 dark:text-white">Delete Supplier</h2></div>
        </div>
        <div className="p-6"><p className="text-gray-600 dark:text-gray-300 mb-4 text-sm">This action cannot be undone. The supplier record will be permanently removed.</p><p className="text-sm font-semibold p-3 rounded-xl border text-gray-900 dark:text-white bg-gray-100 dark:bg-gray-800/50 border-gray-200 dark:border-gray-700/50">{supplier.name} · {supplier.phone}</p></div>
        <div className="p-6 border-t border-gray-200 dark:border-gray-700/50 flex gap-3">
          <button type="button" onClick={onConfirm} className="flex-1 px-4 py-2.5 rounded-xl font-medium text-sm text-white bg-gradient-to-r from-red-600 to-red-500 hover:from-red-700 hover:to-red-600 flex items-center justify-center gap-2"><Trash2 size={15} /> Delete</button>
          <button type="button" onClick={onCancel} className="flex-1 px-4 py-2.5 rounded-xl font-medium text-sm border transition-colors bg-gray-100 dark:bg-gray-700/50 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-900 dark:text-white border-gray-300 dark:border-gray-600/50">Cancel</button>
        </div>
      </div>
    </div>
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="rounded-2xl max-w-md w-full shadow-2xl border overflow-hidden bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700/50 max-h-[90vh] flex flex-col">
        <div className="p-5 border-b shrink-0 bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/10 border-green-200 dark:border-green-700/30">
          <div className="flex items-center gap-4"><div className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0 bg-green-100 dark:bg-green-500/20"><Banknote size={22} className="text-green-600 dark:text-green-400" /></div><div><h2 className="text-lg font-bold text-gray-900 dark:text-white">Settle Payment</h2><p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{supplier.name}</p></div></div>
        </div>
        <form onSubmit={handleSubmit} noValidate className="flex flex-col flex-1 min-h-0 overflow-hidden">
          ...
          <div className="px-5 pb-5 pt-2 flex gap-3 shrink-0 border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900">
            <button type="submit" className="flex-1 px-4 py-2.5 rounded-xl font-semibold text-sm text-white bg-gradient-to-r from-green-500 to-emerald-500 shadow-green-500/20 flex items-center justify-center gap-2 hover:opacity-90"><CheckCircle2 size={15} /> Confirm Payment</button>
            <button type="button" onClick={onCancel} className="flex-1 px-4 py-2.5 rounded-xl font-medium text-sm border transition-colors bg-gray-100 dark:bg-gray-700/50 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-900 dark:text-white border-gray-300 dark:border-gray-600/50">Cancel</button>
          </div>
        </form>
      </div>
...
export default function SuppliersPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return suppliers.filter(s => {
      const matchSearch = !q || s.name.toLowerCase().includes(q) || s.phone.toLowerCase().includes(q) || s.category?.toLowerCase().includes(q)
      const matchPayable = payableFilter === 'all' ? true : payableFilter === 'payable' ? s.payableAmount > 0 : s.payableAmount === 0
  ...
  const totalPayable = suppliers.reduce((sum, s) => sum + (s.payableAmount || 0), 0)
  const withPayableCount = suppliers.filter(s => (s.payableAmount || 0) > 0).length
  const totalPurchases = suppliers.reduce((sum, s) => sum + (s.totalPurchases || 0), 0)
  const hasAnyFilter = search || payableFilter !== 'all'

  async function handleSave(data) {
    if (formTarget?.id) {
      const result = await update(formTarget.id, data)
      if (result.success) { /* toast handled in store */ }
    } else {
      const result = await create(data)
      if (result.success) { resetPage() }
    }
    setFormTarget(null)
  }
  ...
  function clearAll() { setSearch(''); setPayableFilter('all'); resetPage() }
  ...
  return (
    <div className="flex flex-col gap-5 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3">
        <div className="flex items-center gap-3 px-4 py-3 rounded-2xl border-2 bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800"><div className="w-9 h-9 rounded-xl bg-amber-500 flex items-center justify-center shrink-0"><Truck size={16} className="text-white" /></div><div className="min-w-0"><p className="text-xl font-extrabold text-gray-900 dark:text-white tabular-nums">{suppliers.length}</p><p className="text-xs text-gray-500 dark:text-gray-400 truncate">Total Suppliers</p></div></div>
        <button type="button" onClick={() => { setPayableFilter(payableFilter === 'payable' ? 'all' : 'payable'); resetPage() }} className={`flex items-center gap-3 px-4 py-3 rounded-2xl border-2 text-left transition-all active:scale-[0.98] ${payableFilter === 'payable' ? 'bg-red-500/10 border-red-500/40 ring-2 ring-red-500/20' : 'bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800 hover:border-gray-300'}`}>
      ...
      <div className="p-3 sm:p-4 rounded-2xl border bg-white dark:bg-gray-800/30 border-gray-200 dark:border-gray-700/50">
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex items-center gap-2 px-3 py-2 rounded-xl border flex-1 min-w-[200px] bg-gray-50 dark:bg-gray-800/50 border-gray-200 dark:border-gray-700/50">
            ...
            <input type="text" placeholder="Search by name…" value={search} onChange={e => { setSearch(e.target.value); resetPage() }} className="bg-transparent border-none outline-none flex-1 min-w-0 text-sm text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-500" />
            {search && <button type="button" onClick={() => { setSearch(''); resetPage() }} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200"><X size={14} /></button>}
          </div>
          <SearchableSelect options={PAYABLE_FILTER_OPTIONS} value={payableFilter} onChange={v => { setPayableFilter(v); resetPage() }} placeholder="All Suppliers" searchPlaceholder="Filter…" triggerClassName="w-44" />
          {hasAnyFilter && <button type="button" onClick={clearAll} className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold shrink-0 bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700"><X size={12} /> Clear</button>}
        ...
        {hasAnyFilter && <p className="text-xs text-gray-400 dark:text-gray-600 mt-2">Showing {filtered.length} of {suppliers.length} suppliers</p>}
      </div>

      <div className="rounded-2xl border overflow-hidden bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800">
        {viewMode === 'grid' ? (
          pageItems.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 gap-3"><Package size={32} className="text-gray-300 dark:text-gray-700" /><p className="text-sm font-medium text-gray-400 dark:text-gray-600">No suppliers match your filters</p></div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 p-4">
              {pageItems.map(supplier => {
                const hasPayable = (supplier.payableAmount || 0) > 0
                return (
                  <div key={supplier.id} className="group flex flex-col rounded-2xl border overflow-hidden bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800 shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200">
                    <div className={`h-1.5 shrink-0 ${hasPayable ? 'bg-gradient-to-r from-red-400 to-rose-500' : 'bg-gradient-to-r from-amber-400 to-orange-500'}`} />
                    <div className="flex flex-col gap-3 p-4 flex-1">
                      <div className="flex items-center gap-3">
                        <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center text-white text-base font-extrabold shadow-sm border-2 border-amber-200 dark:border-amber-700 shrink-0">
                          {supplier.name?.charAt(0).toUpperCase() || 'S'}
                        </div>
                        <div className="flex-1 min-w-0"><p className="font-bold text-gray-900 dark:text-gray-100 truncate text-sm leading-tight">{supplier.name}</p><p className="text-xs text-gray-500 dark:text-gray-400 truncate mt-0.5">{supplier.phone}</p></div>
                      </div>
            ...
            <table className="w-full text-sm min-w-[720px]">
              <thead>
                <tr className="border-b bg-gray-50 dark:bg-gray-800/60 border-gray-200 dark:border-gray-700/50">
                  {['Supplier', 'Category', 'Phone', 'Total Purchases', 'Payable', 'Actions'].map(h => (<th key={h} className="px-4 py-3 text-left text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest whitespace-nowrap">{h}</th>))}
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {pageItems.length === 0 ? (
                  <tr><td colSpan={6} className="px-4 py-14 text-center"><Package size={28} className="mx-auto mb-2 text-gray-300 dark:text-gray-700" /><p className="text-sm font-medium text-gray-400 dark:text-gray-600">No suppliers match your filters</p></td></tr>
                ) : pageItems.map(supplier => {
                  const hasPayable = (supplier.payableAmount || 0) > 0
                  return (
                    <tr key={supplier.id} className="bg-white dark:bg-gray-900 hover:bg-amber-50/50 dark:hover:bg-gray-800/30 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1 flex-wrap justify-end">
                          <button type="button" onClick={() => setFormTarget(supplier)} className="p-2 rounded-xl text-gray-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-500/10"><Pencil size={15} /></button>
                          <button type="button" onClick={() => setDeleteTarget(supplier)} className="p-2 rounded-xl text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-500/10"><Trash2 size={15} /></button>
                          {hasPayable && <button type="button" onClick={() => setSettleTarget(supplier)} className="relative flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-xs font-semibold text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 hover:bg-green-100"><Banknote size={12} /> Settle{supplier.paymentCount > 0 && <button type="button" onClick={e => { e.stopPropagation(); setHistoryTarget({ ...supplier, _historyType: 'payments' }) }} className="absolute -top-1.5 -right-1.5 w-4 h-4 rounded-full bg-emerald-500 hover:bg-emerald-600 text-white text-[9px] font-extrabold flex items-center justify-center shadow-sm transition-colors active:scale-90">{supplier.paymentCount > 9 ? '9+' : supplier.paymentCount}</button>}</button>}
                        </div>
                      </td>
                    </tr>
                  )
