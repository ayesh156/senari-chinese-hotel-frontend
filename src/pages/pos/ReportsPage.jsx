...
import {
 ...
 import DailySalesSummaryModal from '../../components/pos/DailySalesSummaryModal'
...
async function exportToPDF(summary, revenueChart, topSelling, leastSelling, profitableFoods, periodText) {
  ...
  doc.text('Senari Restaurant', M, 40)
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.text('Senari Restaurant — Confidential', PAGE_W / 2, PAGE_H - 16, { align: 'center' })
  }
...
export default function ReportsPage() {
  ...
  }, [summary, revenueChart, foodRankings, profitableFoods, hourlyTraffic, paymentDistribution])
    ...
    <div className="flex flex-col gap-5 sm:gap-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setShowDailySummary(true)} className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold bg-gradient-to-r from-amber-500 to-orange-500 text-white hover:opacity-90 transition-opacity shadow-sm">
            <ListChecks size={13} /> Today's Summary
          </button>
          <button onClick={handleExportCSV} disabled={loading || exportingCSV} className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold bg-gradient-to-r from-emerald-500 to-teal-600 text-white hover:opacity-90 transition-opacity shadow-sm disabled:opacity-50">
            <Download size={13} /> {exportingCSV ? 'Exporting...' : 'Export CSV'}
          </button>
          <button onClick={handleExportPDF} disabled={loading || exporting} className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold bg-gradient-to-r from-blue-600 to-indigo-700 text-white hover:opacity-90 transition-opacity shadow-sm disabled:opacity-50">
