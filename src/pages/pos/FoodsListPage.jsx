...
// ── FoodThumbnail — robust image with fallback ────────────────────────────────
function FoodThumbnail({ imagePath, alt = 'Food', className = 'w-12 h-12 rounded-xl' }) {
  ...
  return (
    <div className={`${className} overflow-hidden shrink-0 bg-amber-50 dark:bg-gray-800`}>
      <img src={fullUrl} alt={alt} onError={() => setImgError(true)} className="w-full h-full object-cover" />
    </div>
  )
}
...
export default function FoodsListPage() {
  ...
  const { foods, loading, fetchAll: fetchFoods, update: updateFood, remove: deleteFood } = useFoodStore()
  ...
  }, [sortedFoods, search, catFilter, availFilter, priceRange, newOnly, sortKey, sortDir])
  ...
  return (
    <div className="flex flex-col gap-5 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
          <button type="button" onClick={() => navigate('/pos/master-data')}
            className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl font-medium text-sm border transition-colors shrink-0 bg-white dark:bg-gray-800/50 text-gray-700 dark:text-gray-200 border-gray-200 dark:border-gray-700 hover:border-amber-300 dark:hover:border-amber-700 hover:text-amber-700 dark:hover:text-amber-400">
            <Database size={16} /> Master Data
          </button>
          <button type="button" onClick={() => navigate('/pos/foods/add')}
            className="flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-xl font-medium text-sm hover:opacity-90 transition-opacity shadow-md shadow-amber-500/20 shrink-0">
      ...
      {loading && (
        <div className="flex items-center justify-center py-4 gap-2 text-sm text-gray-400 dark:text-gray-500">
          <span className="w-4 h-4 rounded-full border-2 border-gray-300 dark:border-gray-600 border-t-amber-500 animate-spin" />Loading foods…
        </div>
      )}
      ...
      <div className="p-3 sm:p-4 rounded-2xl border bg-white dark:bg-gray-800/30 border-gray-200 dark:border-gray-700/50">
        <div className="flex flex-col gap-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <SearchableSelect options={catSelectOptions} value={catFilter} onChange={v => { setCatFilter(v); resetPage() }} placeholder="All Categories" searchPlaceholder="Search categories…" triggerClassName="w-44" />
            <SearchableSelect options={AVAIL_SELECT_OPTIONS} value={availFilter} onChange={v => { setAvailFilter(v); resetPage() }} placeholder="All Status" searchPlaceholder="Search status…" triggerClassName="w-36" />
            <button onClick={() => setShowAdvancedFilters(v => !v)}
              className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold border transition-all duration-150 shrink-0 ${showAdvancedFilters || hasAdvancedFilters ? 'bg-amber-500 text-white border-amber-500 shadow-sm shadow-amber-500/20' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-400 border-gray-200 dark:border-gray-700 hover:border-amber-300 dark:hover:border-amber-700'}`}>
              <SlidersHorizontal size={13} /> More Options<ChevronDown size={12} className={`transition-transform duration-200 ${showAdvancedFilters ? 'rotate-180' : ''}`} />
            </button>
          ...
          <>
            {pageItems.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 p-4">
                {pageItems.map(item => (
                  <div key={item.id} className="group flex flex-col rounded-2xl border overflow-hidden bg-amber-50 dark:bg-gray-800 border-amber-100 dark:border-gray-700 shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200">
