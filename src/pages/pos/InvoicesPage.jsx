...
export default function InvoicesPage() {
  ...
  // ── Food store (individual selectors — prevents getSnapshot loop) ────────
  const foods = useFoodStore(s => s.foods)
  const fetchAll = useFoodStore(s => s.fetchAll)

  // Fetch foods on mount
  useEffect(() => { fetchAll() }, []) // eslint-disable-line react-hooks/exhaustive-deps

  // ── Invoice store (individual selectors) ─────────────────────────────────
  const orders = useInvoiceStore(s => s.orders)
  const loading = useInvoiceStore(s => s.loading)
  const fetchOrders = useInvoiceStore(s => s.fetchOrders)
  const deleteOrder = useInvoiceStore(s => s.deleteOrder)
  const addInvoiceToList = useInvoiceStore(s => s.addInvoiceToList)
  const updateInvoiceInList = useInvoiceStore(s => s.updateInvoiceInList)
    ...
    <div className="flex flex-col gap-5 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {/* Add Invoice button — opens InvoiceFormModal */}
          <button
