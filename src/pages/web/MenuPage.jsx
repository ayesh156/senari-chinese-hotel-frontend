...
// ── Category Filter Bar ───────────────────────────────────────────────────────
function CategoryFilters({ active, onChange, categories }) {
  return (
...
function FilterSidebarContent({ priceRange, onPrice, newOnly, onNew, healthyOnly, onHealthy, onReset, maxPrice }) {
  const activeCount = (priceRange < maxPrice ? 1 : 0) + (newOnly ? 1 : 0) + (healthyOnly ? 1 : 0)
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-3">
        ...
        <input type="range" min={0} max={maxPrice} step={50}
        ...
        <div className="flex justify-between text-xs text-gray-400 dark:text-gray-500">
          <span>Rs. 0</span><span>{fmtCurrencyDirect(maxPrice)}</span>
        </div>
      </div>
    ...
    <Link to={`/menu/${item.id}`}
      ...
      <div className="flex-1 min-w-0">
        <span className="text-xs font-semibold text-amber-600 dark:text-amber-400
                         ...
                         border border-amber-100 dark:border-amber-800">{categoryName}</span>
        ...
        <div className="flex items-center gap-3 mt-2">
          <span className="flex items-center gap-1 text-xs text-gray-400 dark:text-gray-500">
            <Clock size={11} /> {item.prepTimeMinutes || 15} min
          </span>
          <span className="flex items-center gap-1 text-xs text-orange-400">
            <Flame size={11} /> {item.calories || 450} kcal
          </span>
...
// ─────────────────────────────────────────────────────────────────────────────
// Page — fetches from live API, filters and paginates client-side
// ─────────────────────────────────────────────────────────────────────────────
export default function MenuPage() {
  ...
  useEffect(() => {
    ...
    foodApi.getAll()
      ...
      .catch(err => {
        if (cancelled) return
        console.error('[MenuPage] Failed to fetch foods:', err)
  ...
  // Derived category list from fetched data
  const CATEGORIES = useMemo(() => {
    const cats = new Set(foods.map(f => f.category?.name || f.category || '').filter(Boolean))
    return ['All', ...Array.from(cats)]
  }, [foods])

  // Max price from data
  const MAX_PRICE = useMemo(() => {
    if (!foods.length) return 2000
    return Math.max(...foods.map(i => Number(i.price)), 2000)
  }, [foods])
  ...
  const filteredItems = useMemo(() => {
    let items = activeCategory === 'All'
      ? foods
      : foods.filter(i => (i.category?.name || i.category) === activeCategory)
    ...
    items = items.filter(i => Number(i.price) <= priceRange)
    if (newOnly)     items = items.filter(i => i.isNew)
    if (healthyOnly) items = items.filter(i => (i.calories || 450) < HEALTHY_CAL)
    ...
    if (sortBy === 'calories')   sorted.sort((a, b) => (a.calories || 450) - (b.calories || 450))
    return sorted
  }, [foods, activeCategory, searchQuery, priceRange, newOnly, healthyOnly, sortBy])
  ...
  if (error) {
    return (
      <div className="max-w-6xl mx-auto px-4 py-24 text-center">
        <p className="text-red-500 font-semibold">Failed to load menu. Please try again later.</p>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12">
      ...
      <AnimatedSection delay={0.05} className="mb-6">
        <CategoryFilters active={activeCategory} onChange={v => setParam('category', v)} categories={CATEGORIES} />
      </AnimatedSection>
