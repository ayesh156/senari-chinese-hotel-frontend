...
export default function ReceiptModal({ isOpen, onClose, order }) {
  ...
  // Compute estimated tax & service charge for display
  ...
  const surcharge = Math.max(0, totalNum - afterDiscount)
  const estServiceCharge = Math.round(afterDiscount * 0.1)
  const estTax = Math.max(0, surcharge - estServiceCharge)
