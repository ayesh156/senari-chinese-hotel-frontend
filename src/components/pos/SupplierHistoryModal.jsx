...
export default function SupplierHistoryModal({ supplier, type = 'payments', onCancel }) {
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    const fetcher = type === 'payments' ? supplierApi.getPayments : supplierApi.getReminders;
    fetcher(supplier.id)
      ...
      <div className="rounded-t-2xl sm:rounded-2xl w-full sm:max-w-2xl shadow-2xl border bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700/50 max-h-[85vh] flex flex-col overflow-hidden">
        ...
        <div className={`relative overflow-hidden shrink-0 bg-gradient-to-r ${gradient}`}>
          <div className="relative flex items-center gap-3 px-4 sm:px-5 py-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-white/70 text-xs mt-0.5 truncate">{supplier.name} · {records.length} {isPayments ? 'payment(s)' : 'reminder(s)'}</p>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 min-h-0">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-2"><Icon size={32} className="text-gray-300 dark:text-gray-700" /><p className="text-sm font-medium text-gray-400 dark:text-gray-500">No {isPayments ? 'payments' : 'reminders'} yet</p></div>
          ) : (
              ...
              {pageItems.map((rec) => {
                ...
                return (
                  <div key={rec.id} className="grid grid-cols-[auto_1fr_auto] gap-3 px-3 py-2.5 rounded-xl text-sm transition-colors hover:bg-gray-50 dark:hover:bg-gray-800/30">
                    ...
                    <div className="min-w-0"><p className="text-xs font-medium text-gray-800 dark:text-gray-200 truncate">{rec.notes || rec.message || (isPayments ? `Payment of ${fmtCurrency(rec.amountPaid)}` : 'Reminder sent')}</p></div>
                    <div className="text-right text-xs font-bold tabular-nums whitespace-nowrap">
                      {isPayments ? <span className="text-emerald-600">{fmtCurrency(rec.amountPaid)}</span> : <span className="text-violet-500 capitalize">{rec.status}</span>}
                    </div>
                  </div>
                );
              })}
